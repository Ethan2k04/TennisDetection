...
import queue
from constants import MAX_RETRY, RETRY_INTERVAL
from typing import Dict

# 获取网络接口的 MAC 地址, 优化只取一次
mac_address = ""
# 用于与 推送数据函数线程 通信 
push_data_queue = queue.Queue()
...
def _post_data_to_server(payload:Dict, retry_count:int ,session:requests.Session)->bool:
    ...
    try:
        response = session.post(url, json={})  # 发送 GET 请求，参数已经在 URL 中
...
# 推送数据函数线程
def push_data_worker():
    session = requests.Session()

    while True:
        payload = push_data_queue.get()
        retry_count = 0
        success = False
         # 获取并加入 machine_id 字段
        global mac_address
        ...
        while retry_count <= MAX_RETRY:
            success = _post_data_to_server(payload=payload,retry_count = retry_count,session =session)
            if  success:
                break

            # failure to post data to server, retry 
            retry_count += 1
...
# 推送数据函数
def push_data_async(payload: dict):
    push_data_queue.put(payload)
